function RDBLS2D_frame_right()
%% Step 1: initialize parameters
[tau,dt,dv,nx,ny,Vmax,E,nu] = deal(1e-6,2,0.9,100,100,0.2,1,0.3);tic
N = [1 23 23 1; 23 529 529 23; 23 529 529 23;1 23 23 1]/2304;
initBSLS = ones(ny+3,nx+3); %Initialize the levelset function on nodes.(phin) % expanded outward one round
initBSLS(0.4*end:end,0.4*end:0.8*end)=zeros;
PHIcoeff = generate_coeff_by_fast_interpolation(initBSLS);
phie = getphie(N,PHIcoeff,nx,ny);
x = phie>0;
V0=mean(x(:));
...
F(2:2*(ny+1):2*(nx*(ny+1)+1),1) = -2/(nx+1);%vertical force
F(2*nx*(ny+1)+1:2:2*(ny+1)*(nx+1),1) = (ny:-1:0)./(ny.^2);% horizontal force (leftwards pointing)
freedofs = true(size(F));
freedofs([1,2,2*(nx+1)*(ny+1)]) = false;
...
%% Step 5: update the design variables
    ...
    while (l2-l1>1e-6)
        ...
        phie = getphie(N,PHINew,nx,ny);
        x = phie>=0;
        if mean(x(:)) - V0 > 0 l1 = lambda; else l2 = lambda; end
    end
    PHIcoeff = PHINew/2;
end
function phie = getphie(N,PHIcoeff,nx,ny)
phie=conv2(PHIcoeff,N,'valid');
phie(1:ny/50,:)=max(phie(:));
phie(:,[1:nx/50,end-nx/50+1:end])=max(phie(:));
phie(0.4*ny:end,0.4*nx:end-nx/5)=min(phie(:));%add void domain
